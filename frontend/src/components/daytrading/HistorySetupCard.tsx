      ...
      <summary className="grid cursor-pointer list-none gap-3 px-3 py-3 sm:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_auto] sm:items-center">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded px-1.5 py-0.5 text-2xs font-semibold ${setup.side === 'CALL' ? 'bg-emerald-950/50 text-emerald-300' : 'bg-rose-950/50 text-rose-300'}`}>{setup.side}</span>
            <span className="truncate text-xs font-semibold text-zinc-200">{strategy.name}</span>
          </div>
          <div className="mt-1 truncate font-mono text-2xs text-zinc-500" title={contractName(option, setup.side)}>{contractName(option, setup.side)}</div>
        </div>
        ...
        <div className="flex items-center justify-between gap-3 sm:justify-end">
          <div className="text-right">
            <div className="text-2xs font-semibold text-zinc-300">{outcome}</div>
            <div className="mt-0.5 font-mono text-2xs text-zinc-600">{dateTime(setup.created_at)}</div>
          </div>
          <ChevronDown className="h-4 w-4 shrink-0 text-zinc-600 transition-transform group-open:rotate-180" />
        </div>
      </summary>
      <div className="border-t border-zinc-800 px-3 py-3">
        <div className="mb-3 rounded-md border border-zinc-800 bg-black/15 px-2.5 py-2 text-2xs leading-relaxed text-zinc-400">
        ...
        <div className="mt-3 text-2xs font-semibold uppercase tracking-[0.14em] text-zinc-500">Lifecycle timeline</div>
        <div className="mt-2 space-y-0">
          {setup.lifecycle_events.length > 0 ? setup.lifecycle_events.map((event, index) => (
            <div key={event.id} className="grid grid-cols-[0.8rem_4.5rem_1fr] gap-2 text-2xs">

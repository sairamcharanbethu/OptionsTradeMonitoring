    ...
    <div className="mt-4 rounded-lg border border-zinc-700/80 bg-zinc-950/55 p-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-2xs font-semibold uppercase tracking-[0.14em] text-zinc-500">Planned entry ticket</div>
          <div className="mt-1 text-sm font-semibold text-zinc-100">{humanContractName(option, side)}</div>
          <div className="mt-1 select-all break-all font-mono text-2xs text-zinc-500" title="Contract symbol">{contractName(option, side)}</div>
        </div>
        <span className={`shrink-0 rounded-md border px-2 py-1 text-2xs font-semibold ${
          quoteFresh
      ...
      <div className="mt-3 border-t border-zinc-800 pt-2">
        ...
        <div className="mt-1 font-mono text-2xs tabular-nums text-zinc-500">
          {money(option.bid)} bid / {money(option.ask)} ask · {Number.isFinite(spreadPct) ? `${number(spreadPct)}% spread` : 'spread unavailable'}
        </div>
      </div>
      <div className="mt-2 text-2xs text-zinc-600">Protected limit order only</div>
    </div>
  );
};

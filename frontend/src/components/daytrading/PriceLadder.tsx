  ...
  return (
    <div>
      ...
      <div className={cn('relative rounded-full bg-gradient-to-r from-rose-500/30 via-zinc-700 to-emerald-500/30', compact ? 'h-1.5' : 'h-2')} role="img" aria-label={label}>
        {marks.map((m) => (
          <div key={m.label} className={cn('rail-glide absolute flex -translate-x-1/2 flex-col items-center', compact ? '-top-3.5' : '-top-4')} style={{ left: x(m.price) }}>
        ...
        {spot != null && Number.isFinite(spot) && (
          <div className={cn('rail-glide-live absolute -translate-x-1/2', compact ? '-top-1' : '-top-1.5')} style={{ left: x(spot) }} title={`Spot ${money(spot)}`}>

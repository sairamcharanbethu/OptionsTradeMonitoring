  ...
  return (
    <>
      <div
        ...
        className="fixed inset-x-2 bottom-[calc(4.5rem+env(safe-area-inset-bottom))] z-40 flex flex-nowrap items-center gap-1.5 overflow-x-auto rounded-lg border border-border/70 bg-background/95 p-2 shadow-lg backdrop-blur-xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:static lg:inset-auto lg:ml-auto lg:w-auto lg:overflow-visible lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none lg:backdrop-blur-none"
      >
        <span className="flex shrink-0 items-center gap-1.5 pl-1 text-2xs font-semibold uppercase tracking-wide text-muted-foreground" title={connected ? 'Live push connected' : 'Reconnecting — falling back to polling'}>
        ...
        <StatusChip severity={statusChip.severity} className="shrink-0" aria-live="polite">{statusChip.text}</StatusChip>
        ...
        {setupVetoed ? (
          <button
            type="button"
            className="flex h-10 min-w-[5.5rem] shrink-0 items-center justify-center gap-1.5 rounded-md border border-border px-2.5 sm:min-w-[7.5rem] sm:px-3 text-xs font-semibold text-foreground transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:border-act-disabled-line disabled:bg-act-disabled-bg disabled:text-act-disabled-fg"
            onClick={unveto}

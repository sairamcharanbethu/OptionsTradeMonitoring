...
const backtests_1 = require("./routes/backtests");
const manual_entry_1 = require("./routes/manual-entry");
const paper_account_1 = require("./routes/paper-account");
const position_monitor_1 = require("./routes/position-monitor");
const kill_switch_1 = require("./routes/kill-switch");
const metrics_1 = require("./routes/metrics");
const mcp_1 = require("./routes/mcp");
const lean_shadow_1 = require("./routes/lean-shadow");
const jwt_1 = __importDefault(require("@fastify/jwt"));
const auth_1 = __importDefault(require("./routes/auth"));
const admin_1 = require("./routes/admin");
const snaptrade_1 = require("./routes/snaptrade");
const adapter_health_1 = require("./lib/adapter-health");
...
loadEnvFile(path_1.default.join(__dirname, '../../.env'));
loadEnvFile(path_1.default.join(process.cwd(), '.env'));
...
function streamQuotePayload(quote) {
...
const ensureSchema = async (instance) => {
    ...
    `);
        const retiredCalendarSetting = await instance.pg.query(`DELETE FROM settings WHERE key = $1`, ['trading_economics_api_key']);
    ...
    `);
        ...
        try {
            ...
            const legacy = await ledgerClient.query(`SELECT id FROM paper_accounts WHERE id IN ('strategy-system', 'wall-reaction-system') FOR UPDATE`);
            if (legacy.rows.length) {
                const applied = await ledgerClient.query(`INSERT INTO paper_ledger_migrations (migration_key) VALUES ('shared-paper-account-v1') ON CONFLICT DO NOTHING RETURNING migration_key`);
                if (applied.rows[0]) {
...
const start = async () => {
    try {
        ...
        fastify.register(trades_1.tradeRoutes, { prefix: '/api/trades' });
        fastify.register(signals_1.signalRoutes, { prefix: '/api/signals' });
        fastify.register(backtests_1.backtestRoutes, { prefix: '/api/backtests' });
        fastify.register(manual_entry_1.manualEntryRoutes, { prefix: '/api/manual-entry' });
        fastify.register(goals_1.goalRoutes, { prefix: '/api/goals' });
        fastify.register(mcp_1.mcpRoutes);
        ...
        const { IbkrMarketDataService } = await Promise.resolve().then(() => __importStar(require('./services/ibkr-market-data-service')));
        const ibkrMarketData = new IbkrMarketDataService(fastify);
        fastify.decorate('ibkrMarketData', ibkrMarketData);
        fastify.register(position_monitor_1.positionMonitorRoutes, { prefix: '/api/position-monitor' });
        fastify.register(kill_switch_1.killSwitchRoutes, { prefix: '/api/kill-switch' });
        fastify.register(metrics_1.metricsRoutes, { prefix: '/api/metrics' });
        ...
        const { StrategyEngineAdapter } = await Promise.resolve().then(() => __importStar(require('./services/strategy-engine-adapter')));
        const { PaperTradingService } = await Promise.resolve().then(() => __importStar(require('./services/paper-trading-service')));
        ...
        const { LeanShadowService } = await Promise.resolve().then(() => __importStar(require('./services/lean-shadow-service')));
        ...
        fastify.register(lean_shadow_1.leanShadowRoutes, { prefix: '/api' });
        fastify.register(paper_account_1.paperAccountRoutes, { prefix: '/api/paper-account' });
        ...
        const { IbkrMarketDataStreamService } = await Promise.resolve().then(() => __importStar(require('./services/ibkr-market-data-stream-service')));
        ...
        const { LiveExitMonitorService } = await Promise.resolve().then(() => __importStar(require('./services/live-exit-monitor-service')));
        const liveExitMonitor = new LiveExitMonitorService(fastify);
        fastify.decorate('liveExitMonitor', liveExitMonitor);
        const { OptionMarketHistoryCaptureService } = await Promise.resolve().then(() => __importStar(require('./services/option-market-history-capture-service')));
        ...
        const { SnaptradeService } = await Promise.resolve().then(() => __importStar(require('./services/snaptrade-service')));
        const snaptradeOrderSync = new SnaptradeService(fastify);
        const { OrderWatchdogService } = await Promise.resolve().then(() => __importStar(require('./services/order-watchdog-service')));
        const orderWatchdog = new OrderWatchdogService(fastify);
        const { TradeRedisService } = await Promise.resolve().then(() => __importStar(require('./services/trade-redis-service')));
        const snaptradePendingOrderSyncHealth = {
            status: 'IDLE',
            running: false,
            lastRunAt: null,
            lastResult: null,
            lastWatchdogResult: null,
            lastError: null,
            intervalSeconds: Number(process.env.SNAPTRADE_PENDING_SYNC_INTERVAL_SECONDS || 15),
            redisRehydratedAt: null,
            redisRehydratedUsers: 0,
            queuedSyncLastRunAt: null,
            queuedSyncProcessed: 0
        };
        const hydrateTradeRedisReadModels = async () => {
            const { rows } = await fastify.pg.query(`SELECT DISTINCT user_id
         FROM positions
        ...
        const manualQuoteSubscriptions = new Map();
        const manualQuoteSubscriptionKeys = new Map();
        const removeManualQuoteSubscriptions = (socket) => {
        ...
        const subscribeManualQuote = async (socket, clientId, payload) => {
        ...
        const handleStreamQuote = async (quote) => {
        ...
        fastify.get('/api/services/health', { preHandler: fastify.authenticate }, async (request) => {
            ...
            const resolvedIbkrConfig = await (0, ibkr_config_1.getIbkrGatewayConfig)(fastify.pg).catch(() => ({
            ...
            const [ibkrHealth, scannerHealth, tradeRedisHealth, postgresHealth] = await Promise.all([
                withTimeout(ibkrMarketData.getHealth().catch((err) => ({
                    status: 'DOWN',
                    connected: false,
                    provider: 'ibkr',
                ...
                withTimeout(scanner.getRuntimeStatus(), 1500, () => ({
                    status: 'DEGRADED',
                    enabled: false,
                    marketOpen: false,
                    error: 'Scanner health check timed out'
                })),
                withTimeout(TradeRedisService.getHealth(), 1500, () => ({
                    status: 'DEGRADED',
                    connected: false,
                    queueDepth: null,
                    metrics: {},
                ...
                withTimeout(fastify.pg.query('SELECT 1')
                    .then(() => (0, adapter_health_1.normalizeAdapterHealth)('postgres', {
                    status: 'UP',
                    latencyMs: Date.now() - postgresStartedAt,
                    lastError: null
                }, generatedAt))
                    .catch((err) => (0, adapter_health_1.normalizeAdapterHealth)('postgres', {
                    status: 'DOWN',
                    latencyMs: Date.now() - postgresStartedAt,
                    lastError: err.message || String(err)
                }, generatedAt)), 1500, () => (0, adapter_health_1.normalizeAdapterHealth)('postgres', {
                    status: 'DOWN',
                    latencyMs: Date.now() - postgresStartedAt,
            ...
            return {
                liveExitMonitor: (0, adapter_health_1.normalizeAdapterHealth)('liveExitMonitor', liveExitHealth, generatedAt),
                optionHistoryCapture: (0, adapter_health_1.normalizeAdapterHealth)('optionHistoryCapture', optionHistoryHealth, generatedAt),
                streams: {
                    ibkr: (0, adapter_health_1.normalizeAdapterHealth)('ibkrStream', ibkrStreamHealth, generatedAt)
                },
                marketData: {
                    ibkr: (0, adapter_health_1.normalizeAdapterHealth)('ibkr', ibkrHealth, generatedAt)
                },
                poller: (0, adapter_health_1.normalizeAdapterHealth)('marketPoller', {
                    status: poller.isRunning() ? 'UP' : 'DOWN',
                    running: poller.isRunning()
                }, generatedAt),
                marketDataBuffer: (0, adapter_health_1.normalizeAdapterHealth)('marketDataBuffer', poller.getMarketDataBufferHealth(), generatedAt),
                scanner: (0, adapter_health_1.normalizeAdapterHealth)('signalScanner', scannerHealth, generatedAt),
                strategyEngine: (0, adapter_health_1.normalizeAdapterHealth)('strategyEngine', {
                ...
                leanShadow: (0, adapter_health_1.normalizeAdapterHealth)('leanShadow', leanShadowHealth, generatedAt),
                paperTrading: (0, adapter_health_1.normalizeAdapterHealth)('paperTrading', paperTrading.getHealth(), generatedAt),
                snaptradePendingOrders: (0, adapter_health_1.normalizeAdapterHealth)('snaptradePendingOrders', snaptradePendingOrderSyncHealth, generatedAt),
                tradeRedis: (0, adapter_health_1.normalizeAdapterHealth)('tradeRedis', tradeRedisHealth, generatedAt),
        ...
        const getLegacyWsClientId = (req) => {
            const fingerprint = [
                req.ip || req.headers?.['x-forwarded-for'] || 'unknown',
                req.headers?.['user-agent'] || 'unknown',
                req.headers?.['accept-language'] || 'unknown'
            ].join('|');
            return `legacy-${crypto_1.default.createHash('sha1').update(fingerprint).digest('hex').slice(0, 10)}`;
        };
        const getActiveWsCountForClient = (clientId) => {
            let count = 0;
        ...
        fastify.get('/api/ws', { websocket: true }, (socket, req) => {
            ...
            fastify.log.info(`[WebSocket] Client connected id=${clientId} legacy=${!hasBrowserClientId} active=${wsClients.size} activeForClient=${activeForClient} remote=${req.ip || 'unknown'}`);
            socket.on('message', (message) => {
                try {
                    ...
                    if (data && data.type === 'auth') {
                        try {
                            const verified = fastify.jwt.verify(data.token || '');
                    ...
                    if (data && data.type === 'MANUAL_ENTRY_SUBSCRIBE_QUOTE') {
                        ...
                        subscribeManualQuote(socket, clientId, data.data).catch((err) => {
        ...
        const startBackgroundServices = async () => {
            ...
            runSnaptradePendingOrderSync().catch((err) => {
